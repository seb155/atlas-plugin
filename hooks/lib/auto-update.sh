...
atlas_auto_update_plugins() {
  ...
  if [ "$current_branch" != "main" ]; then
    printf '🆙 ATLAS v%s disponible (installed v%s) — source repo on %s, skipped (switch to main to auto-update)\n' \
      "$mp_version" "$inst_version" "${current_branch:-unknown}"
    return 0
  fi

  # Guard: source repo must be clean (no uncommitted changes)
  if ! git -C "$source_repo" diff --quiet 2>/dev/null || ! git -C "$source_repo" diff --cached --quiet 2>/dev/null; then
    printf '🆙 ATLAS v%s disponible (installed v%s) — source repo has uncommitted changes, skipped\n' \
